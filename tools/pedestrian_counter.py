...
class LineCounter:
    def __init__(self, start: Point, end: Point):
        ...
        self.prev_direction: Dict[str, str] = {}  # Add this line
        self.in_count: int = 0
    ...
    def update(self, detections: Detections):
        ...
        for xyxy, confidence, class_id, tracker_id in detections:
            ...
            if tracker_id not in self.tracker_state:
                ...
                self.prev_direction[tracker_id] = None  # Add this line
                continue
            ...
            # Modify the counting logic here
            if tracker_state:
                if self.prev_direction[tracker_id] == "down":
                    self.out_count -= 1  # Change this line
                else:
                    self.in_count += 1
                self.prev_direction[tracker_id] = "up"
            else:
                if self.prev_direction[tracker_id] == "up":
                    self.in_count -= 1  # Change this line
                else:
                    self.out_count += 1
                self.prev_direction[tracker_id] = "down"
...
class LineCounterAnnotator:
    def __init__(
        ...
        self.write_in_out: float = write_in_out
        
    def annotate(self, frame: np.ndarray, line_counter: LineCounter, write_in_out=False) -> np.ndarray:
        """
        ...
        in_text =  f"Yukari: {line_counter.in_count}"
        out_text = f"Asagi : {line_counter.out_count}"
